...
public class IFractalService {
	...
	public static List<Atividade> loadAtividadesFromIFractalv2(int ano, int mes) {
		...
		try {
			...
			while ((strCurrentLine = objReader.readLine()) != null) {
				...
				if (strCurrentLine.contains(toCompare) && !strCurrentLine.contains("Espelho")) {
					Atividade atividade = new Atividade();
					int beginIndex      = 0;
					int endIndex        = 10;
					String strDate      = strCurrentLine.substring(beginIndex, endIndex);
					...
					strCurrentLine = strCurrentLine.substring(strCurrentLine.indexOf("e	"));
